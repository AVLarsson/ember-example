import { symbol, takeAssociated, peekAssociated, snapshot, destructor, DESTROY, Stack, clearElement, WILL_DROP, DID_DROP, initializeGuid, fillNulls, isHandle, decodeHandle, dict, unreachable, assign, EMPTY_ARRAY, associate, LinkedList, decodeImmediate, isSmallInt, encodeImmediate, encodeHandle, associateDestructor, isDrop, ListSlice } from '@glimmer/util';
import { ConstReference, IterableImpl, CachedReference, isModified, ReferenceCache, ReferenceIterator, IteratorSynchronizer, END } from '@glimmer/reference';
...
import { createCombinatorTag, CONSTANT_TAG, value, validate, isConst, isConstTag, combine, createUpdatableTag, update, INITIAL } from '@glimmer/validator';
import { unwrapTemplate, unwrapHandle } from '@glimmer/opcode-compiler';
import { PrimitiveReference } from '@glimmer/runtime';
import { Stack as Stack$1 } from '@glimmer/low-level'; // These symbols represent "friend" properties that are used inside of
// the VM in other classes, but are not intended to be a part of
// Glimmer's API.

const INNER_VM = symbol('INNER_VM');
const DESTRUCTOR_STACK = symbol('DESTRUCTOR_STACK');
...
class NewElementBuilder {
  ...
  appendDynamicHTML(value$$1) {
    let bounds = this.trustedContent(value$$1);
    this.didAppendBounds(bounds);
  }

  appendDynamicText(value$$1) {
    let node = this.untrustedContent(value$$1);
    this.didAppendNode(node);
    return node;
  }

  appendDynamicFragment(value$$1) {
    let bounds = this.__appendFragment(value$$1);

    this.didAppendBounds(bounds);
  }

  appendDynamicNode(value$$1) {
    let node = this.__appendNode(value$$1);
...
function sanitizeAttributeValue(env, element, attribute, value$$1) {
  ...
  let str = normalizeStringValue(value$$1);
...
class SimpleDynamicAttribute extends DynamicAttribute {
  set(dom, value$$1, _env) {
    let normalizedValue = normalizeValue(value$$1);
...
class SafeDynamicProperty extends DefaultDynamicProperty {
  set(dom, value$$1, env) {
    ...
    let sanitized = sanitizeAttributeValue(env, element, name, value$$1);
...
class InputValueDynamicAttribute extends DefaultDynamicProperty {
  ...
  update(value$$1) {
    ...
    let normalizedValue = normalizeStringValue(value$$1);
...
class ConcatReference extends CachedReference {
  ...
  compute() {
    ...
    for (let i = 0; i < this.parts.length; i++) {
      let value$$1 = this.parts[i].value();
...
, (vm, {
  ...
  let args = stack.pop();
  let value$$1 = helper(args, vm);
  vm.loadValue($v0, value$$1);
});
...
class ClassListReference {
  ...
  value() {
    ...
    for (let i = 0; i < list.length; i++) {
      let value$$1 = normalizeStringValue(list[i].value());
...
, vm => {
  ...
  let value$$1 = isEmpty(rawValue) ? '' : String(rawValue);
...
, vm => {
  ...
  let value$$1 = isEmpty(rawValue) ? '' : rawValue;
  vm.elements().appendDynamicHTML(value$$1);
});
...
, vm => {
  ...
  let rawValue = reference.value();
  let value$$1 = isEmpty(rawValue) ? '' : String(rawValue);
  let node = vm.elements().appendDynamicText(value$$1);
...
, (vm, {
  ...
  if (isHandle(primitive)) {
    ...
    /* NUMBER_MAX_HANDLE */
        ...
        value$$1 = vm[CONSTANTS].getString(decodeHandle(primitive, -1
        /* STRING_MAX_HANDLE */
        ));
      } else {
      value$$1 = vm[CONSTANTS].getNumber(decodeHandle(primitive, -1073741825
      /* NUMBER_MAX_HANDLE */
      ));
    }
...
, (vm, {
  ...
  let name = vm[CONSTANTS].getString(_name);
  let value$$1 = vm[CONSTANTS].getString(_value);
...
, (vm, {
  ...
  let value$$1 = reference.value();
  let namespace = _namespace ? vm[CONSTANTS].getString(_namespace) : null;
  let attribute = vm.elements().setDynamicAttribute(name, value$$1, !!trusting, namespace);
...
, (vm, {
  ...
  let value$$1 = vm[CONSTANTS].getString(_value);
  let namespace = _namespace ? vm[CONSTANTS].getString(_namespace) : null;
  vm.fetchValue($t0).setStaticAttribute(name, value$$1, namespace);
});
...
function makeClassList(classes) {
  for (let i = 0; i < classes.length; i++) {
    const value$$1 = classes[i];

    if (typeof value$$1 === 'string') {
      classes[i] = PrimitiveReference.create(value$$1);
    }
  }
...
function setDeferredAttr(vm, name, value$$1, namespace, trusting = false) {
  if (typeof value$$1 === 'string') {
    ...
    let attribute = vm.elements().setDynamicAttribute(name, value$$1.value(), trusting, namespace);
...
, (vm, {
  ...
  for (let i = callerNames.length - 1; i >= 0; i--) {
    ...
    let value$$1 = args.named.get(atName, true);
...
class InnerStack {
  ...
  write(pos, value$$1) {
    switch (typeof value$$1) {
      case 'boolean':
      case 'undefined':
        this.writeRaw(pos, encodeImmediate(value$$1));
        break;

      case 'number':
        if (isSmallInt(value$$1)) {
          this.writeRaw(pos, encodeImmediate(value$$1));
          break;
        }
  ...
  get(pos) {
    let value$$1 = this.inner.getRaw(pos);

    if (isHandle(value$$1)) {
      return this.js[decodeHandle(value$$1)];
    } else {
...
class EvaluationStackImpl {
  ...
  static restore(snapshot$$1) {
    ...
    for (let i = 0; i < snapshot$$1.length; i++) {
  ...
  push(value$$1) {
    this.stack.write(++this[REGISTERS][$sp], value$$1);
  }

  pushJs(value$$1) {
    this.stack.writeJs(++this[REGISTERS][$sp], value$$1);
  }

  pushRaw(value$$1) {
    this.stack.writeRaw(++this[REGISTERS][$sp], value$$1);
  }
  ...
  set(value$$1, offset, base = this[REGISTERS][$fp]) {
...
class VM {
  ...
  commitCacheGroup() {
    let END$$1 = new LabelOpcode('END');
    let opcodes = this.updating();
    let marker = this[STACKS].cache.pop();
    let head = marker ? opcodes.nextNode(marker) : opcodes.head();
    let tail = opcodes.tail();
    let tag = combineSlice(new ListSlice(head, tail));
    let guard = new JumpIfNotModifiedOpcode(tag, END$$1);
...
class RehydrateBuilder extends NewElementBuilder {
  ...
  __pushRemoteElement(element, cursorId, insertBefore) {
    let marker = this.getMarker(element, cursorId); // when insertBefore is not present, we clear the element
...
export { clear, ConcreteBounds, CursorImpl, capabilityFlagsFrom, hasCapability, resetDebuggerCallback, setDebuggerCallback, CurriedComponentDefinition, curry, isCurriedComponentDefinition, DEFAULT_CAPABILITIES, MINIMAL_CAPABILITIES, helper$1 as DOMChanges, DOMChangesImpl as IDOMChanges, DOMTreeConstruction, isWhitespace, normalizeProperty, DefaultDynamicScope, AotRuntime, EnvironmentImpl, CustomJitRuntime, JitRuntimeFromProgram, JitRuntime, ScopeImpl, inTransaction, getDynamicVar, ConditionalReference, NULL_REFERENCE, PrimitiveReference$1 as PrimitiveReference, UNDEFINED_REFERENCE, renderAot, renderAotComponent, renderAotMain, renderJitComponent, renderJitMain, renderSync, UpdatingVM, VM as LowLevelVM, EMPTY_ARGS, CapturedArgumentsImpl, CapturedNamedArgumentsImpl, CapturedPositionalArgumentsImpl, DynamicAttribute, dynamicAttribute, SimpleDynamicAttribute, clientBuilder, NewElementBuilder, UpdatableBlockImpl, RemoteLiveBlock, isSerializationFirstNode, RehydrateBuilder, rehydrationBuilder, SERIALIZATION_FIRST_NODE_STRING, SimpleComponentManager, TEMPLATE_ONLY_COMPONENT };