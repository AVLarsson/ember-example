...
import { setListeners, getDebugName, isObject, setupMandatorySetter, symbol, toString, setWithMandatorySetter, Cache, HAS_NATIVE_PROXY, isEmberArray, isProxy, lookupDescriptor, inspect, getName, setName, getListeners, getObservers, getOwnPropertyDescriptors, guidFor, makeArray, ROOT, setObservers, wrap } from '@ember/-internals/utils';
...
import { combine, CURRENT_TAG, validate, value, CONSTANT_TAG, dirtyTagFor, setAutotrackingTransactionEnv, setPropertyDidChange, tagFor, ALLOW_CYCLES, createUpdatableTag, update, consume, deprecateMutationsInAutotrackingTransaction, isTracking, track, untrack, trackedData } from '@glimmer/validator';
...
const CUSTOM_TAG_FOR = symbol('CUSTOM_TAG_FOR'); // This is exported for `@tracked`, but should otherwise be avoided. Use `tagForObject`.
...
function tagForProperty(obj, propertyKey) {
  ...
  if (DEBUG) {
    setupMandatorySetter(tag, obj, propertyKey); // TODO: Replace this with something more first class for tracking tags in DEBUG
...
function tagForObject(obj) {
  if (isObject(obj)) {
    if (DEBUG) {
      let meta$$1 = meta(obj);
      assert(meta$$1.isMetaDestroyed() ? `Cannot create a new tag for \`${toString(meta$$1.source)}\` after it has been destroyed.` : '', !meta$$1.isMetaDestroyed());
    }
...
function makeComputedDecorator(desc, DecoratorClass) {
  let decorator = function COMPUTED_DECORATOR(target, key, propertyDesc, maybeMeta, isClassicDecorator$$1) {
    assert(`Only one computed property decorator can be applied to a class field or accessor, but '${key}' was decorated twice. You may have added the decorator to both a getter and setter, which is unnecessary.`, isClassicDecorator$$1 || !propertyDesc || !propertyDesc.get || propertyDesc.get.toString().indexOf('CPGETTER_FUNCTION') === -1);
...
function descriptorForField([_target, key, desc]) {
  ...
  return {
    ...
    get() {
      let value$$1 = getter(this); // Add the tag of the returned value if it is an array, since arrays
      // should always cause updates if they are consumed and then changed
